...
public class LunCopyBeanCapabilityConvert {
    public S2DLunCopyBean convertSouth2Model(SLunCopyBean southItem) {
        if (null == southItem) {
            return null;
        }

        S2DLunCopyBean modelItem = new S2DLunCopyBean();
        //common tag
        ...
        //private tag
        modelItem.setLUNCOPYIDS(southItem.getLUNCOPYIDS());

        return modelItem;
    }

    public List<S2DLunCopyBean> convertSouth2Model(List<SLunCopyBean> southItem) {
        List<S2DLunCopyBean> modelItem = new ArrayList<S2DLunCopyBean>();
        if (null == southItem) {
            return modelItem;
        }

        for (SLunCopyBean ss : southItem) {
